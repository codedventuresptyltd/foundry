...
Price calculation with multi-stage modifiers. All pricing APIs are now consolidated under the `/catalog/` endpoint.
...
**API Endpoints:**
- `POST /catalog/price` — Calculate pricing for line items
- `GET /catalog/:snapshotId/rules` — List pricing rules (filter by `ruleOverlay`)
...
Inventory and delivery management. All fulfillment APIs are now consolidated under the `/catalog/` endpoint.
...
**API Endpoints:**
- `POST /catalog/fulfillment` — Calculate fulfillment cards
- `GET /catalog/:snapshotId/inventory` — Get inventory (supports warehouse filter)
- `GET /catalog/:snapshotId/warehouses` — Get warehouse configurations
