...
Modifiers are grouped by `ruleOverlay` - a grouping key that identifies rules sharing the same base pricing context.
...
Pricing operations are accessed via the unified `/catalog/` endpoint:

| Endpoint | Purpose |
|----------|---------|
| `POST /catalog/price` | Calculate pricing for line items |
| `GET /catalog/:snapshotId` | Get snapshot details |
| `GET /catalog/:snapshotId/atoms` | List atoms (base prices) in snapshot |
| `GET /catalog/:snapshotId/bindings` | List bindings in snapshot |
| `GET /catalog/:snapshotId/rules` | List rules in snapshot (filter by `ruleOverlay`) |
| `GET /catalog/inspect/trace` | Trace pricing calculation |
