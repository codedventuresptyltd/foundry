...
- Warehouse-specific inventory filtering
...
Fulfillment operations are accessed via the unified `/catalog/` endpoint:

| Endpoint | Purpose |
|----------|---------|
| `POST /catalog/fulfillment` | Calculate fulfillment cards for items |
| `GET /catalog/:snapshotId/availability` | Get availability data for snapshot |
| `GET /catalog/:snapshotId/warehouses` | Get fulfillment configurations |
| `GET /catalog/:snapshotId/inventory` | Get inventory data (supports warehouse filter) |
| `PUT /catalog/:snapshotId/inventory` | Update inventory data |
