...
## PreSearch Extraction

PreSearch is the extraction layer that parses query terms before search execution. Previously called "Facets" in the parsing context, PreSearch configs define:

- **Parsing rules** — How to extract structured values from query text
- **Field mappings** — Which index fields to target
- **Boost weights** — Relative importance of extracted terms

PreSearch extraction runs before the main search, identifying patterns like:
- Brand names ("Nike shoes" → brand: Nike)
- Categories ("industrial valves" → category: industrial)
- Attributes ("red 2-inch pipe" → color: red, size: 2-inch)

The search quality calculation uses PreSearch parsing configs to determine how well a query matched the extraction rules.
...
Define available sort methods (configurable via sorting config):
...
Sorting configurations can be customized per search config to provide different sort options for different product types or categories.
